        ...
        if i % save_interval == 0:
            ...
            if an['intrinsic'] != None:
                save_point_cloud(pcd.reshape((-1, 3)), rgb_origin.reshape(-1, 3),
                                 osp.join(save_pcd_dir, an['folder'], an['filename'][:-4] + '.ply'))
            else:
                ...
                for r in [1.0]:
                    for f in [1000]:
                        # f_scale = f
                        f_scale = f * (rgb_origin.shape[0] + rgb_origin.shape[1]) / (
                                    cfg.data_basic.canonical_space['img_size'][0] +
                                    cfg.data_basic.canonical_space['img_size'][1])
